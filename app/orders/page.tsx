...
export default function Orders(): React.JSX.Element {
  ...
  useEffect(() => {
    (async () => {
      ...
      console.log(await allPositions);
  ...
  const rows = [
    {
      ...
      asset: 'BTC',
      type: 'Buy',
      feeBalance: '0.003 ETH / 141.06 USDC',
      orderBalance: '45.03 / 56.006 USDC',
  ...
  return (
    <div className="flex flex-col h-screen flex flex-col items-center">
      <div className="p-[20px] xl:w-[1262px] w-11/12 border-1-solid-#3D59AD rounded-[15px] bg-[#7980A580] mt-[110px]">
        <DataGrid columns={columns} rows={rows} className="MuiDataGrid-autosizing"/>
      </div>
    </div>
  );
}
